...
2. **By failed step:**
   ...
   | **Prune stale aquatic events** | DB connection or stats-error from `prune_stale_aquatic` | Same DB-secret-stale check as the load step. The pruner returns non-zero only if it caught an exception during `q.delete(...)`; check the printed error and confirm `DATABASE_URL` points at the live Postgres. Hard-delete is scoped to `source_url LIKE '%lhcaz.gov/parks-recreation/open-swim-schedule%'` AND `date < today - 7 days` — WebTrac and admin events are never touched. |
