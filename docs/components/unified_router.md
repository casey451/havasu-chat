...
No items in `BACKLOG.md` currently target unified_router directly — open items there are about Tier 2 ranking, parser context injection, and clickable URLs (now closed). The router is the path those features flow through, but it isn't the seat of the work.
...
**Indirect (called by handlers, not by the router itself):** Tier 2 internals (`tier2_parser`, `tier2_db_query`, `tier2_formatter`, `tier2_catalog_render`, `tier2_schema`), Tier 3 internals (`context_builder`, `local_voice_matcher`), Tier 1 internals (`tier1_templates`). These get their own component docs as the decomposition proceeds; the router is agnostic to their internals.
