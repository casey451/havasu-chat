...
- **Output artifacts** (paths per `confabulation-eval-runbook.md` §Artifacts):
  ...
  - `scripts/confabulation_eval_results/halt3-baseline/per_row.csv` — `row_id`, `row_name`, `category`, `activity_category`, `total_runs`, `included_runs`, `gating_runs_with_hit`, `advisory_token_count`, `top_3_gating_tokens` (9 columns; `category`/`activity_category` populated per row type via #64 harness v2, 2026-05-11)
...
- **Anchor set composition:** Canonical anchor list lives at the module-top constant `_DEFAULT_REGRESSION_ANCHORS` in `app/eval/confabulation_report.py` (since #64 ship 2026-05-11; previously inline at the original `:167-173`). Two-name allowlist: `Aqua Beginnings` (Provider) and `Grace Arts Live` (Program). Names matched case-insensitively against `row_name`. `_PROBES_PROVIDER` / `_PROBES_PROGRAM` define the probe templates applied to every live row, not the anchor allowlist. Per-category regression at §3.2 granularity now reads the `category` (Provider) and `activity_category` (Program) columns directly from `per_row.csv` via #64 harness v2 — no post-process join required.
- **Anchor set governance note:** anchor set (the two-name allowlist) is governed by HALT 1 (owner-reviewed, append-only, no silent drift) per `confabulation-eval-runbook.md` §Lexicon Governance. The default allowlist is the module-top `_DEFAULT_REGRESSION_ANCHORS` in `app/eval/confabulation_report.py`; confirm it is unchanged between baseline run SHA and close-out SHA. If a richer allowlist is in use (via `--anchor-set-file relay/halt3-anchor-set.txt` or similar), confirm that file is unchanged between baseline run SHA and close-out SHA.
...
- **Per-category methodology:** Read the `category` column (Provider rows) and `activity_category` column (Program rows) directly from `per_row.csv` (per #64 harness v2 ship, 2026-05-11). The seven categories are the operator-enrichment-sprint categories (`halt3_definition.md` §6.1): restaurants, plumbers, HVAC, pool service, boat repair, urgent care, auto repair. Emergency-tagged categories per strategy doc §1.1: plumbers, urgent care.
...
- **Per-row offender ranking source:** column `gating_runs_with_hit` in `per_row.csv` (per `confabulation-eval-runbook.md` §Artifacts); `top_3_gating_tokens` provides the qualitative signal per row. Column names are hard-coded in `app/eval/confabulation_report.py::write_per_row_csv` as the 9-column literal header `row_id, row_name, category, activity_category, total_runs, included_runs, gating_runs_with_hit, advisory_token_count, top_3_gating_tokens` (since #64 ship 2026-05-11; was 7 columns prior); spot-check the generated CSV header matches this exactly.
