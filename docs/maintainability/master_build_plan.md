...
- **Phase 2B.1 — pending.** Photos schema + Cloudflare R2 client + Pillow pipeline + upload route. Per brief §5. Gated on operator-side R2 setup (Cloudflare bucket + API token + Railway env vars per `outputs/operator_prereqs_phase_2.md` §2 — ~30-45 min). Dispatch prompt pre-positioned at `outputs/cursor_dispatch_prompt_phase_2b_1.md`. Note: 2B.1's upload-auth depends on Phase 2A.3's `viewer_is_owner` + claim flow (now shipped on origin), so 2B.1 is unblocked the moment R2 is locked.
- **Phase 2B.3 — pending.** Search bar UI + `GET /api/search` endpoint + Lane 2B close-out. Per brief §7. Gated on Phase 2B.2 ship (now done — FTS infrastructure available). Dispatch prompt pre-positioned at `outputs/cursor_dispatch_prompt_phase_2b_3.md`. Independent of 2B.1 (search bar doesn't need photos); can ship before or after 2B.1.
