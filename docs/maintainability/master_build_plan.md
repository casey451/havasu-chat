...
- **Phase 2A.3 — pending.** Final sub-phase: claim flow (`/claim/<slug>`) + admin claim review queue + `viewer_is_owner` plumbing + favorites + admin role parallel-path + close-out. Per brief §7. Dispatch prompt pre-positioned at `outputs/cursor_dispatch_prompt_phase_2a_3.md`.
- **Phase 2A.3 — pending.** Claim flow (`/claim/<slug>`) + favorites + admin role parallel-path + `viewer_is_owner` wiring + close-out. Per brief §7.
