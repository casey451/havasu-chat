...
- **Phase 2A.2 — pending.** Next sub-phase: auth flow routes (`/login`, `/api/auth/request-link`, `/auth/callback`, `/logout`) + session middleware + `require_user` dependency + login templates + rate limits on `/api/auth/request-link`. Per brief §6.
