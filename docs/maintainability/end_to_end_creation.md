...
**Events**: No standalone "admin direct create event" form in admin/router.py. Events come via Path 1, Path 2, or the public-facing `POST /events` endpoint (rate-limited 5/min — see `docs/maintainability/http_api.md` for the posture-verify note).
