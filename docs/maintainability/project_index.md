...
**`app/contrib/`** — Cross-cutting ingestion and helpers: **`river_scene.py`** (fetch/parse/normalize River Scene listings), **`river_scene_pull.py`** (`run_pull` used by CLI), **`approval_service.py`** (pending contribution → `Provider` / `Program` / `Event` rows), **`webtrac.py`** (parser + fetcher for the Vermont Systems WebTrac registration system at `register.lhcaz.gov` — registerable parks-and-rec activities with availability state), **`lhcaz_aquatic.py`** (parser + fetcher for the Aquatic Center weekly class schedule at `lhcaz.gov/parks-recreation/open-swim-schedule`), **`scrape_runner.py`** (fail-soft snapshot orchestrator — runs registered `pull_snapshot()` callables and writes timestamped JSON + manifest under `data/scrapes/`), **`parks_rec_loader.py`** (snapshot → `Contribution` → auto-approve as `Event` for single-day or `Program` for recurring; tags rows with content category + audience; dedup via normalized `source_url`), **`enrichment.py`**, **`mention_scanner.py`**, **`hours_helper.py`**, **`url_fetcher.py`**, **`places_client.py`**, **`event_date_line.py`**, etc.
...
**`scripts/`** — Operational CLIs and one-off tools: **`river_scene_pull.py`** (thin wrapper over **`app.contrib.river_scene_pull.run_pull`**), **`run_scrapes.py`** (top-level scrape orchestrator — runs every registered source via **`scrape_runner`** and writes snapshots), **`webtrac_pull.py`** (single-source CLI with **`--offline`** mode for fixture testing), **`parks_rec_load.py`** (read latest snapshot per source and load into the catalog through the contribution + approval flow; **`--dry-run`** counts without writing), **`parks_rec_prune.py`** (hard-delete stale aquatic Event rows whose `source_url` matches the aquatic schedule page AND whose `date` is older than `today - 7 days`; supports **`--dry-run`** and **`--grace-days N`**; safe-by-construction — WebTrac and admin events are never matched), **`parks_rec_reset_and_reload.py`** (one-off purge + reload after changing tag/category logic), backfills, query batteries, confabulation result dirs, voice audit JSON; **`scripts/README.md`** summarizes several entries. **`alembic/`** — migration versions and env for schema changes.
...
**Parks-rec scrapers → database (scheduled)** — **`scripts/run_scrapes.py`** dispatches to **`app/contrib/scrape_runner.py`**, which calls each registered source's **`pull_snapshot()`** (currently **`webtrac.fetch_all_sections`** + **`lhcaz_aquatic.pull_snapshot`**) and writes timestamped JSON to **`data/scrapes/<source>/<ts>.json`** plus a rolling manifest at **`data/scrapes/manifest.json`**. **`scripts/parks_rec_load.py`** then reads the newest snapshot per source via **`scrape_runner.load_latest_records`** and routes each record through **`app/contrib/parks_rec_loader.py`** — single-day WebTrac sections become `Event` rows, multi-day or weekly-recurring sections become `Program` rows, aquatic class slots become per-occurrence `Event` rows; only publicly bookable rows enter the catalog (`available_for_signup` / `is_public`). Dedup is by normalized **`source_url`** so re-runs are idempotent. After load, **`scripts/parks_rec_prune.py`** invokes **`app.contrib.parks_rec_loader.prune_stale_aquatic`** to hard-delete aquatic Event rows older than `today - 7 days` (the aquatic page republishes a rolling ~25-day window, so old occurrences would otherwise pile up forever; chat hides them via date filter but the row count drifts). The full pipeline runs every 6 hours via the GitHub Actions cron at **`.github/workflows/parks-rec-scrapes.yml`** (scrape → load → prune), which hits production Postgres through the **`DATABASE_URL`** repo secret. **See also:** `docs/scrapes.md` (full pipeline reference, schedule wirings, failure modes).
...
| `docs/scrapes.md` | Parks-rec scrape pipeline reference: WebTrac + Aquatic Center sources, snapshot pattern, scheduler wirings (cron / Procfile / GH Actions), recipe for adding a new scraper. | Current |
