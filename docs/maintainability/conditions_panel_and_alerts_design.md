...
> **Resolution status note (2026-05-19):** Phase 8a's live prereq verification + 3 wrapper-amendment waves (§6 + §11 + §12 in `outputs/phase_8a_prereq_verification_report.md`) closed 5 of these 10 open questions. Specifically: Q1 USGS gauge, Q2 lake surface temp (partial), Q4 LHC emergency feed, Q8 AirNow zip + distance, Q10 AirNow API key. Each is marked RESOLVED below with the cross-reference. Q3, Q5, Q6, Q7, Q9 remain operator-decisions or stable design locks.

1. **[RESOLVED 2026-05-19]** ~~USGS gauge ID for Lake Havasu.~~ Locked: **single site `09427500`** ("Lake Havasu near Parker Dam") with parameters **`00065`** (gauge height ft) + **`00054`** (reservoir storage ac-ft). Original recommendation included `00010` (water temp) — **NOT AVAILABLE at 09427500** (V1.5 carry for alt sources per Q2 below). Original recommendation included site `09427520` — **DROPPED, historic-only since September 2006** (verified empty values array with method note `[Historic 10/1987 to 09/2006]`). See `outputs/phase_8a_prereq_verification_report.md §1 + §2 + §3`.

2. **[RESOLVED-PARTIAL 2026-05-19]** ~~Lake surface temperature source.~~ Closed for V1; **V1.5 carry**. USGS 09427500 doesn't report `00010`. Candidates in decreasing-feasibility order: (a) USGS site `09426630` (Bill Williams River at Lake Havasu; browser-verify pending); (b) Bureau of Reclamation Lower Colorado Region gauges; (c) marina sensor partnership (Lake Havasu Marina or similar); (d) operator-typed via admin form (the original §2.4 Layer 4 fallback). V1 ships without lake surface temp; V1.5 picks a source. See `outputs/phase_8a_prereq_verification_report.md §11.1` for the V1.5 carry.

3. **Heat-advisory trigger threshold.** Fire on NWS-issued heat advisory only? OR also on bare `heat_index_f >= 110` even without an NWS alert? OR both (whichever fires first)? Recommendation: NWS-issued only. NWS is the authority; firing on a private threshold risks contradicting them and reducing trust. Confirm. — **Status:** wrapper locked NWS-issued only (per `outputs/cursor_dispatch_prompt_phase_8.md` §3 deliverable (f)); operator can enable bare-threshold via `HEAT_ADVISORY_INDEX_FLOOR_F` env var post-launch.

4. **[RESOLVED 2026-05-19]** ~~Lake Havasu City emergency-notification feed.~~ Closed: **Nixle DROPPED from V1.** LHC Fire Department uses Nixle (agency `3726`), confirmed live + valid RSS at `https://rss.nixle.com/pubs/feeds/latest/3726/` — but the feed has been **silent since 2021-09-01** (~4 years 8 months of zero posts; historical posts are 18-of-20 internal "RECALL FOR RESIDENTIAL ASSIGNMENT" staff-callback messages, not public-safety alerts). LHC Police has NO Nixle presence (single municipal agency listed). **V1.5 carry to research alt sources:** Mohave County Sheriff's Office alerting platform, Arizona Emergency Information Network (`ein.az.gov`), `lhcaz.gov` RSS, AZ DPS alerts. Phase 8a's `lake_hazard` trigger reframed to use NWS-AZZ002-zone keyword match (inland-LHC product set) + USGS gauge-drop heuristic — no Nixle dependency. See `outputs/phase_8a_prereq_verification_report.md §1 + §4 + §11`.
...
8. **[RESOLVED 2026-05-19]** ~~AirNow zip-code coverage.~~ Locked: **single ZIP `86403`** with **`AIRNOW_DISTANCE_MI = 100`** default (operator-tunable env var). §12 verification confirmed LHC has NO AirNow monitor within 60mi; **nearest is Blythe CA at ~60mi south** (single-parameter O3 only; no PM2.5/PM10 within 100mi). Multi-zip coverage would surface the same Blythe station for all three ZIPs — no marginal value. Honest source-station attribution chip ("from Blythe, CA ~60mi south") renders alongside the AQI value per the §12 wrapper amendments. See `outputs/phase_8a_prereq_verification_report.md §12`.
...
10. **[RESOLVED 2026-05-19]** ~~AirNow API key registration.~~ DONE. Registered under `casey.l.solomon@gmail.com`; activated near-instantly via email-activation flow (no 1-2 business day lag). Key saved to local `.env` 2026-05-19. **Railway env var write pending Railway outage recovery** (Google Cloud account-block 2026-05-19). Smoke-test confirmed key works at `distance=100`; returns Blythe CA O3 single row.
