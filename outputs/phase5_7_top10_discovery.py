"""Discover the top-10 outdoors-parks-trails entities by review_count
and dump their Provider.google_review_snippets to JSON for the
``apply_phase5_7_parks_crowd_notes.py`` drafting step.

Read-only against ``data/events.db``. Produces
``outputs/phase5_7_top10_data.json`` for the Cowork agent to Read
and hand-curate short+long crowd_notes from.

Usage:
    python outputs/phase5_7_top10_discovery.py
"""
...
def main() -> int:
    out_path = (
        Path(__file__).resolve().parent / "phase5_7_top10_data.json"
    )
    with SessionLocal() as session:
        cat = session.scalar(
            select(Category).where(Category.slug == "outdoors-parks-trails")
        )
        if cat is None:
            print("ERROR: Category.slug='outdoors-parks-trails' not found.")
            return 2
        ...
        for e, p in session.execute(q).all():
            ...
            # Defensive: if it was stored as a stringified JSON blob,
            # try to decode it. Per app/db/models.py:84 it's a JSON
            # column so SQLAlchemy returns the parsed structure already,
            # but the defensive path mirrors how the 5.6 audit dump
            # handles cache-vs-DB shape drift.
