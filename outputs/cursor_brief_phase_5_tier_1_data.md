...
| §3.1.a Beauty / personal care home | LOCKED — skip in Phase 5 (V1.5 defer). `hair_salon` / `beauty_salon` / `nail_salon` explicitly map to `(None, None)` in `google_types_mapping.py` so they don't surface as accidental `eat-drink` rows; operator queue surfaces them for V1.5 review rather than silently ignoring. | Prereq §3.1.a / commit `62ab3b7` |
| §3.1.b RV-park vs lodging boundary | LOCKED — `rv_park` stays in `lodging-vacation-rentals` (where-you-stay framing is the dominant resident-question shape); `rv_repair` lives under `auto-rv-fuel` as part of the auto-RV bundle. Existing `google_types_mapping.py:763` mapping unchanged. | Prereq §3.1.b / commit `62ab3b7` |
| §3.1.c Eat & Drink Google `types[]` coverage | LOCKED — `google_types_mapping.py` expanded pre-dispatch: +13 Eat & Drink (`meal_delivery`, `meal_takeaway`, `fast_food_restaurant`, `dessert_shop`, `wine_bar`, `pub`, `pizza_restaurant`, `seafood_restaurant`, `mexican_restaurant`, `breakfast_restaurant`, `barbecue_restaurant`, `coffee_shop`, `ice_cream_shop`). Also expanded for other 5 Tier 1 categories (+3 on-the-water / +10 home-property / +9 health / +6 auto / +10 shopping = ~54 new entries total). | Prereq §3.1.c / commit `62ab3b7` |
| §3.2.d Per-category run sequencing | LOCKED — per-category (discover → enrich → load → review → next category), not all-at-once batches. Smaller blast radius; operator reviews one category's reconciler output before scaling to the next. | Prereq §3.2.d / new-chat session 2026-05-13 |
| §3.2.e OSM coverage scope | LOCKED — `leisure=marina` + `man_made=pier` + `natural=beach` for the `on-the-water` category only. Other OSM `(tag, value)` pairs (`leisure=park`, `highway=path`, `highway=track`, etc.) deferred to V1.5 or as-surfaces. | Prereq §3.2.e / new-chat session 2026-05-13 |
| §3.2.f Reconciler `ambiguous` queue triage | LOCKED — direct DB query after each scrape (`SELECT * FROM entities WHERE source LIKE 'google_places,osm%' ORDER BY created_at DESC LIMIT 50`). `log_ambiguous_reconcile` already structures log lines so grep over `places_load` output is a workable secondary path for ~20-40 ambig hits per category. No admin form in Phase 5 — admin surface is Phase 6 / V1.5. | Prereq §3.2.f / new-chat session 2026-05-13 |
| §3.3.g `heat_exposure` rubric | LOCKED — operator brainstorms priority-30 list (parks / marinas / patio-prevalent restaurants / outdoor civic venues / dog parks) during lead-up before first scrape (~30 min operator work); tags those entities during field entry; everything else defaults to `indoor`. Phase 8 alert venue-context consumes whatever Phase 5 produces. | Prereq §3.3.g / new-chat session 2026-05-13 |
| §3.3.h `crowd_notes` rubric | LOCKED — short-form (1-sentence per venue) by default for typical entries; long-form (multi-paragraph) only for the top-20 highest-volume venues (Aquatic Park, English Village restaurants, Lake Havasu State Park, top-5 marinas, top-5 grocery/big-box anchors). Per Opus design §5 example. | Prereq §3.3.h / new-chat session 2026-05-13 |
| §3.3.i `boat_access` per-venue JSON shape | LOCKED — `docs/operations/boat_access_rubric.md` authored with 4 canonical shapes (marina / public_ramp / beach / shoreline_commercial) + key tables + NULL-vs-empty-vs-populated semantics + 4 illustrative LHC examples + operator entry tips + Phase 6 consumer reference (~250 lines). Phase 6 boat-mode toggle + Phase 5 §3.2 on-the-water playbook both consume this shape. | Prereq §3.3.i / commit `b755b03` |
| §3.4.j `manual_recovery_checklist.md` back-fill | LOCKED — back-filled ~250 lines across §1-§6 with field-work prompts per Layer-5 sub-category (38 sub-categories across community rec / public infrastructure / hobbyist / ephemeral / non-business / Tier-2). §7 field-trip planner extended with 6 geographic route clusters + sequencing recommendation (Lakefront first for boat-mode-critical density). | Prereq §3.4.j / commit `b755b03` |
| §3.4.k `places_discovery.py` stale-reference patch | LOCKED — docstring repointed from non-existent `relay/HAVA_BUSINESSES_EXECUTION_PLAN_2026-05-06.md` to `outputs/cursor_brief_phase_5_tier_1_data.md` §3 (per-category playbooks) + `outputs/phase5_prereq_checklist.md` §4 (external data-source verifications). | Prereq §3.4.k / commit `62ab3b7` |

Lead-up §3 lock batch sealed in this session (new-chat post-`b755b03`, 2026-05-13). Locks land via three substantive commits: `62ab3b7` (Tier 1 mapping expansion + beauty skip + RV-park boundary + docstring fix), `b755b03` (boat_access rubric + manual_recovery_checklist back-fill), plus this session's brief §2 + prereq §3 doc-state updates. No further §3 work remains; the 5 still-open operator decisions (§3.2.d, §3.2.e, §3.2.f, §3.3.g, §3.3.h) were locked at the operator-chosen recommendation via AskUserQuestion this chat.
