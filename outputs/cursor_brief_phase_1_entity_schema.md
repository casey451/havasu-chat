...
- **Don't write SQLite-only constructs in migrations.** The dev/test environment runs SQLite (and the bash sandbox does too); production runs Railway Postgres. SQLite is permissive about type coercion in ways Postgres is not, so it's possible to write a migration that passes `pytest` + `alembic upgrade head` locally and crashloops production. Specific rules absorbed from session-15's Phase 1A hotfix (`5132162`):
  - **Use `sa.true()` / `sa.false()` (NOT `sa.text("1")` / `sa.text("0")`) for Boolean `server_default` values.** SQLite coerces text "1"/"0" to a boolean at write time; Postgres rejects with `column "..." is of type boolean but default expression is of type text`. `sa.true()` / `sa.false()` are Alembic's portable boolean literal helpers — emit `TRUE`/`FALSE` on Postgres and `1`/`0` on SQLite. Same caution for any other typed `server_default` (use `sa.func.now()` not `sa.text("CURRENT_TIMESTAMP")` for default timestamps; use the typed Python literal not a text wrapper for default integers).
  - **Verify any raw SQL inside `op.execute()` works on Postgres, not just SQLite.** SQLite is loose about quoting (double quotes can act as string literals; `||` for concat works but so do other things), keyword strictness, NULL-handling in unique constraints, and JSON syntax. If you write raw SQL, mentally translate to Postgres syntax before committing — or use SQLAlchemy Core constructs that handle dialect translation for you.
  - **The Phase 1B `_sql_time` SQLite normalization helper is a precedent** — Phase 1B's `app/db/entity_backfill.py` factored out time-bind handling specifically because raw `datetime.time` binds behaved differently between dialects. Look for similar dialect divergence before shipping new dialect-touching code.
  - Cross-reference: §12 risk #5 ("SQLite vs Postgres divergence on cascade deletes or JSON columns") — the boolean default coercion is a third instance of this risk class beyond the two listed there.
