"""Discover the top-10 classes-sports-recreation (cat-12) entities by
review_count and dump their Provider.google_review_snippets to JSON for
the ``apply_phase5_9_classes_crowd_notes.py`` drafting step.

Read-only against ``data/events.db``. Produces
``outputs/phase5_9_top10_data.json`` for the Cowork agent to Read
and hand-curate short+long crowd_notes from.

Mirrors ``outputs/phase5_8_top10_discovery.py`` exactly with slug
swap (events -> classes-sports-recreation).

Usage:
    python outputs/phase5_9_top10_discovery.py
"""
...
def main() -> int:
    out_path = (
        Path(__file__).resolve().parent / "phase5_9_top10_data.json"
    )
