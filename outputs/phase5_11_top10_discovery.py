"""Discover the top-10 pets (cat-11) entities by review_count and dump
their Provider.google_review_snippets to JSON for the
``apply_phase5_11_pets_crowd_notes.py`` drafting step.

Read-only against ``data/events.db``. Produces
``outputs/phase5_11_top10_data.json`` for the Cowork agent to Read
and hand-curate short+long crowd_notes from.

Mirrors ``outputs/phase5_10_top10_discovery.py`` exactly with slug
swap (lodging-vacation-rentals -> pets).

Per kickoff 4, pet-service review snippet coverage forecast 70-90%
(chains abundant, independents lower); the 5.11 1 + 2 surfaced 38
total cat-11 entries with review counts ranging from 0 (Obedience
Please / PetSmart Grooming / PetSmart Dog Training / Penney's
Pampered Pawz / TagWorks) to 347 (Dorita's Place). Expected top-10
all have >= 20 reviews -- decent snippet density.

Usage:
    python outputs/phase5_11_top10_discovery.py
"""
...
def main() -> int:
    out_path = (
        Path(__file__).resolve().parent / "phase5_11_top10_data.json"
    )
