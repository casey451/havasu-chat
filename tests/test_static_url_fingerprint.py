...
def test_appends_content_hash_for_real_asset() -> None:
    css = _STATIC_ROOT / "styles" / "lake_redesign.css"
    assert css.is_file(), "fixture asset missing"
    out = static_url("/static/styles/lake_redesign.css")
    m = re.fullmatch(r"/static/styles/lake_redesign.css\?v=([0-9a-f]{8})", out)
    assert m, out
...
def test_existing_query_uses_ampersand() -> None:
    out = static_url("/static/styles/lake_redesign.css?foo=1")
