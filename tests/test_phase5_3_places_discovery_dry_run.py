...
def test_dry_run_with_category_returns_nonzero_for_every_slug(slug: str) -> None:
    ...
    cats = load_categories_for_discovery(CATEGORIES_PATH, dry_run=True, category_slug=slug)
    assert len(cats) > 0, (
...
def test_dry_run_eat_drink_category_returns_exactly_two_preview_labels() -> None:
    ...
    cats = load_categories_for_discovery(CATEGORIES_PATH, dry_run=True, category_slug="eat-drink")
    assert len(cats) == 2
...
def test_dry_run_without_category_returns_legacy_5_label_sample() -> None:
    ...
    cats = load_categories_for_discovery(CATEGORIES_PATH, dry_run=True, category_slug=None)
