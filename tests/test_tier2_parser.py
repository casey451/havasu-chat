...
def _parse_with_mock(llm_text: str, query: str = "dummy") -> object:
    ...
    with patch.dict(os.environ, {"ANTHROPIC_API_KEY": "test-key"}):
        with patch.object(anthropic, "Anthropic", return_value=fake_client):
            return parse(query), fake_client
...
def test_high_conf_what_should_i_do_saturday() -> None:
    ...
    filters, _ = _parse_with_mock(json.dumps(payload), "what should i do saturday")
...
def test_high_conf_things_to_do_this_weekend() -> None:
    ...
    filters, _ = _parse_with_mock(json.dumps(payload), "things to do this weekend")
    _assert_high_conf(filters)
    dow = filters.day_of_week or []
    weekend_dow = "saturday" in dow and "sunday" in dow
...
def test_high_conf_family_activities_this_month() -> None:
    ...
    filters, _ = _parse_with_mock(json.dumps(payload), "family activities this month")
    _assert_high_conf(filters)
...
def test_high_conf_your_favorite_event_coming_up() -> None:
    ...
    filters, _ = _parse_with_mock(json.dumps(payload), "your favorite event coming up")
    _assert_high_conf(filters)
...
def test_high_conf_events_tomorrow() -> None:
    ...
    filters, fake_client = _parse_with_mock(json.dumps(payload), "events tomorrow")
    _assert_high_conf(filters)
...
def test_high_conf_stuff_at_sara_park() -> None:
    ...
    filters, _ = _parse_with_mock(json.dumps(payload), "stuff happening at sara park")
    _assert_high_conf(filters)
...
def test_high_conf_tell_me_about_bridge_city() -> None:
    ...
    filters, _ = _parse_with_mock(json.dumps(payload), "tell me about bridge city")
    _assert_high_conf(filters)
    ent = (filters.entity_name or "").lower()
...
def test_fallback_whats_the_vibe_here() -> None:
    ...
    filters, _ = _parse_with_mock(json.dumps(payload), "what's the vibe here")
    assert filters is not None
    assert filters.fallback_to_tier3 is True or filters.parser_confidence < 0.7
...
def test_fallback_anything_fun() -> None:
    ...
    filters, _ = _parse_with_mock(json.dumps(payload), "anything fun")
    assert filters is not None
    assert filters.fallback_to_tier3 is True or filters.parser_confidence < 0.7
