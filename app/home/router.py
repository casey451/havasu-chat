"""Hava — ``GET /home`` route (BUILD.md steps 1-4).
...
Step 4: Hava's read pullquote — SWR cache backed by an LLM call.
        See ``app.home.pullquote`` for cache semantics.
...
the mocked Havasu Outdoor Co. card. The pullquote always generates (or
falls back to a fact-free default); never blocks the page.
"""
...
from app.home import mock_data, pullquote, queries, sponsor_store
...
def serve_home(request: Request, db: Session = Depends(get_db)) -> HTMLResponse:
    ...
    # Hava's read: SWR-cached LLM-generated quote. Fast path is in-memory.
    base["hava_read"] = pullquote.get_quote(db)
