"""Re-shelve mortgage brokers mis-filed under ``banks-and-credit-unions``.

Audit 2026-07-06 (T3.3): moves mortgage brokers / loan officers to the dedicated
``mortgage-lenders`` leaf (created by ``scripts/create_mortgage_leaf_2026_07_06.py``
— run that FIRST). Mirrors ``scripts/recategorize_car_wash_detailers``: repoints
only the PRIMARY entity_categories link, scoped to rows whose current primary leaf
is ``banks-and-credit-unions``, one transaction, JSON rollback snapshot first.

DEFAULT IS DRY-RUN. ``--apply`` requires ``--confirm``.

    .venv\\Scripts\\python.exe scripts\\recategorize_mortgage_brokers_2026_07_06.py                    # DRY RUN
    .venv\\Scripts\\python.exe scripts\\recategorize_mortgage_brokers_2026_07_06.py --apply --confirm  # writes
"""
...
from app.categories.mortgage_misfiled_rules import classify_mortgage_misfiled_leaf  # noqa: E402
from app.db.database import DATABASE_URL, SessionLocal  # noqa: E402
from app.db.models import Category, EntityCategory, Provider  # noqa: E402

_SOURCE_LEAF = "banks-and-credit-unions"
...
def run(*, apply: bool = False, confirm: bool = False, session=None) -> Counter:
    ...
    try:
        ...
        for p in providers:
            ...
            desired = classify_mortgage_misfiled_leaf(p.provider_name)
            if desired is None:
                continue
            new_leaf_id = slug_to_id.get(desired)
            if new_leaf_id is None:
                counts["unresolved_leaf"] += 1
                print(f"  (target leaf {desired!r} missing — run create_mortgage_leaf first) {p.provider_name!r}")
                continue
        ...
        out_path = _ROOT / f"recategorize_mortgage_brokers_{datetime.now(UTC):%Y%m%dT%H%M%SZ}.csv"
        ...
        for r in change_rows:
            print(f"  - {r['name'][:44]:44}  {r['old_leaf']} -> {r['new_leaf']}")
        ...
        snap_path = _ROOT / f"recategorize_mortgage_brokers_snapshot_{stamp}.json"
