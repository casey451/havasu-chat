"""Session 6d — final decisions cleanup (GATED, dry-run default).

Three op types over a hand-confirmed decision list, each entity resolved BY NAME
from the banked source CSVs (never hand-keyed IDs):

  * REPOINT   — swap the entity's PRIMARY EntityCategory onto a valid level-1
    leaf (mirrors apply_browse_orphans; swap/promote/repoint/insert), optionally
    also setting the website if empty.
  * SET-WEBSITE — set Provider.website (or a website ContactPoint for a
    place) only when currently empty; never overwrites / never edits a
    verified/claimed/sponsored listing.
  * DEACTIVATE — Entity.is_active=False + cascade every Provider.is_active=False
    (mirrors apply_flagged_cleanup; skips verified/claimed/sponsored/inactive).

Sources (name column): browse_orphans_review (provider_name), the eatdrink +
beauty_tattoo website CSVs (name). A name matching 0 or >1 rows across the three
is reported and skipped (no guessing). Repoint is corrective, so a protected row
is repointed anyway and flagged (it should still be visible on the right leaf);
the website/deactivate paths skip protected rows.
...
PROD GATE (CLAUDE.md): dry-run -> counts -> Casey approves -> apply.
    ...
    .venv\\Scripts\\python.exe scripts/apply_final_decisions_2026_07_05.py
    .venv\\Scripts\\python.exe scripts/apply_final_decisions_2026_07_05.py --apply --undo-csv undo_final_decisions_2026-07-05.csv
    .venv\\Scripts\\python.exe scripts/apply_final_decisions_2026_07_05.py --reactivate-from undo_final_decisions_2026-07-05.csv --apply
"""
...
_CSVS = (
    ...
    (_AUDIT / "completeness_beauty_tattoo_websites_filled_2026-07-05.csv", "name"),
)
_WEB_KINDS = {"website", "web", "url"}

# (name, target_leaf_slug, website_or_None)
_REPOINT: tuple[tuple[str, str, str | None], ...] = (
    ("Tech One Racing", "boat-repair-and-service", "https://techoneracing.com"),
    ("CELIE.CO", "auto-marine-detailing", "https://www.instagram.com/celie.co_/"),
    ("Desert Hawks RC Club", "hobby-and-craft", None),
    ("Lake Havasu City BMX", "parks-and-playgrounds", None),
)
# (name, website)
_SET_WEBSITE: tuple[tuple[str, str], ...] = (
    ("Jacklyn's Nail Studio",
     "https://www.facebook.com/p/Jacklyns-Salon-Specialists-100064877760429/"),
)
_DEACTIVATE: tuple[str, ...] = (
    "Havasu", "Beautiful", "Farn stand goods", "Tapusallc", "TG Kutter",
    "Go Lake Havasu (CVB)", "Complexions, Clinical Skin Care and Waxing Studio",
    "Queen Aesthetic", "Heart of Gold Fine Line Tattoo and Art Studio",
    "The Guided Hand Tattoo Studio",
)
...
class Plan:
    name: str
    op: str                       # repoint | set-website | deactivate | skip
    detail: str = ""
    skip_reason: str = ""
    entity_id: str = ""
    # repoint
    ...
    mode: str = ""
    # website (may ride along a repoint)
    web_target: str = ""          # provider | contact_point | ""
    web_provider_id: str = ""
    web_prior_website: str = ""
    web_new_website: str = ""
    web_cp_id: int | None = None   # a matching aggregator web CP to overwrite too
    web_cp_prior: str = ""
    web_note: str = ""
    # deactivate
...
def _plan_website(pl: Plan, db, eid: str, url: str, provs: list, cps: list,
                  prot: str | None, overwrite: bool = False) -> None:
    """Fill website fields on ``pl`` (only if empty + not protected, unless
    ``overwrite`` — then replace an existing aggregator value and record it)."""
    active = [p for p in provs if p.is_active and not p.draft]
    cur_prov = next((p for p in active if (p.website or "").strip()), None)
    cur = cur_prov.website.strip() if cur_prov else ""
    cp = _web_cp(cps)
    ...
    if prot is not None:
        ...
        return
    if active:
        target_p = cur_prov or sorted(
            active, key=lambda p: -(getattr(p, "google_review_count", 0) or 0))[0]
        pl.web_target = "provider"
        pl.web_provider_id = target_p.id
        pl.web_prior_website = target_p.website or ""
        pl.web_new_website = url
        # On overwrite, also drop a web ContactPoint that mirrors the old value.
        if overwrite and cp is not None and _norm(cp.value) == _norm(target_p.website or ""):
            pl.web_cp_id = cp.id
            pl.web_cp_prior = cp.value or ""
        ovr = f" (overwrite {pl.web_prior_website})" if overwrite and pl.web_prior_website else ""
        cpn = " +CP" if pl.web_cp_id else ""
        pl.web_note = f"Provider.website ← {url}{ovr}{cpn}"
    else:
...
def _build_plans(db, held: set[str], overwrite_web: set[str]) -> list[Plan]:
    ...
    # ---- REPOINT (+ optional website) ----
    for name, slug, url in _REPOINT:
        if _norm(name) in held:
            plans.append(Plan(name, "skip", skip_reason="HELD by Casey (left active/as-is)"))
            continue
        ...
        if prim is not None and prim.category_id == target.id:
            # already on target — but still allow the website set if present.
            pl = Plan(name, "repoint", entity_id=eid, mode="already-at-target",
                      detail=f"already on {slug}")
        else:
        ...
        if url:
            prot = _protected(eid, provs_by_entity.get(eid, []), claimed, now)
            _plan_website(pl, db, eid, url, provs_by_entity.get(eid, []),
                          db.query(ContactPoint).filter(ContactPoint.entity_id == eid).all(),
                          prot, overwrite=_norm(name) in overwrite_web)
        plans.append(pl)

    # ---- SET-WEBSITE only ----
    for name, url in _SET_WEBSITE:
        if _norm(name) in held:
            plans.append(Plan(name, "skip", skip_reason="HELD by Casey (left as-is)"))
            continue
        eid, err = _resolve(name)
        if eid is None:
            plans.append(Plan(name, "skip", skip_reason=err))
        ...
        pl = Plan(name, "set-website", entity_id=eid)
        _plan_website(pl, db, eid, url, provs, cps, prot, overwrite=_norm(name) in overwrite_web)
        if not pl.web_target:
            plans.append(Plan(name, "skip", entity_id=eid, skip_reason=pl.web_note or "website not set"))
        else:
            pl.detail = pl.web_note
            plans.append(pl)

    # ---- DEACTIVATE ----
    for name in _DEACTIVATE:
        if _norm(name) in held:
            plans.append(Plan(name, "skip", skip_reason="HELD by Casey (left active)"))
            continue
        eid, err = _resolve(name)
        if eid is None:
            plans.append(Plan(name, "skip", skip_reason=err))
...
_UNDO_FIELDS = ["op", "name", "entity_id", "prim_ec_id", "old_category_id", "target_ec_id",
                "new_category_id", "mode", "web_target", "web_provider_id", "web_prior_website",
                "web_new_website", "web_cp_id", "web_cp_prior", "provider_ids"]
...
def _undo_row(pl: Plan) -> dict:
    ...
    if pl.op == "repoint":
        row.update(prim_ec_id=pl.prim_ec_id or "", old_category_id=pl.old_category_id or "",
                   ...
                   mode=pl.mode)
    if pl.web_target:
        row.update(web_target=pl.web_target, web_provider_id=pl.web_provider_id,
                   web_prior_website=pl.web_prior_website, web_new_website=pl.web_new_website,
                   web_cp_id=(pl.web_cp_id or ""), web_cp_prior=pl.web_cp_prior)
    if pl.op == "deactivate":
...
def _apply_plan(db, pl: Plan) -> None:
    ...
    if pl.web_target == "provider":
        ...
        if pl.web_cp_id is not None:
            cp = db.get(ContactPoint, pl.web_cp_id)
            if cp is not None:
                cp.value = pl.web_new_website
...
def run(apply: bool, undo_csv: str | None, held: set[str], overwrite_web: set[str]) -> int:
    redacted = DATABASE_URL.split("@")[-1] if "@" in DATABASE_URL else DATABASE_URL
    with SessionLocal() as db:
        plans = _build_plans(db, held, overwrite_web)
        by = {"repoint": [], "set-website": [], "deactivate": [], "skip": []}
        for pl in plans:
            by[pl.op].append(pl)

        mode = "APPLY (writing)" if apply else "DRY RUN (no writes)"
        print("=" * 88)
        print(f"FINAL DECISIONS CLEANUP — Session 6d — {mode}")
        print("=" * 88)
        print(f"DB target: …@{redacted}")
        websites = sum(1 for pl in plans if pl.web_target)
        print(f"repoint: {len(by['repoint'])}   set-website(only): {len(by['set-website'])}   "
              f"deactivate: {len(by['deactivate'])}   skip: {len(by['skip'])}   "
              f"(websites set incl. repoint: {websites})\n")

        print(f"--- REPOINT ({len(by['repoint'])}) ---")
        for pl in by["repoint"]:
            web = f"   +web: {pl.web_note}" if pl.web_note else ""
            print(f"  {pl.name[:34]:34s} {pl.entity_id[:8]}  {pl.detail}{web}")
        print(f"\n--- SET-WEBSITE ({len(by['set-website'])}) ---")
        for pl in by["set-website"]:
            print(f"  {pl.name[:34]:34s} {pl.entity_id[:8]}  [{pl.web_target}] {pl.detail}")
        print(f"\n--- DEACTIVATE ({len(by['deactivate'])}) ---")
        for pl in by["deactivate"]:
            print(f"  {pl.name[:40]:40s} {pl.entity_id[:8]}  {pl.detail}")
        print(f"\n--- SKIPPED ({len(by['skip'])}) ---")
        for pl in by["skip"]:
            print(f"  {pl.name[:40]:40s} — {pl.skip_reason}")
        ...
        actionable = by["repoint"] + by["set-website"] + by["deactivate"]
        ...
        db.commit()
        print(f"\nAPPLIED: repoint={len(by['repoint'])} set-website={len(by['set-website'])} "
    ...
    with SessionLocal() as db:
        ...
        for r in csv.DictReader(open(undo_csv, newline="", encoding="utf-8")):
            ...
            if r.get("web_target") == "provider" and apply:
                ...
                if r.get("web_cp_id"):
                    cp = db.get(ContactPoint, int(r["web_cp_id"]))
                    if cp is not None:
                        cp.value = r.get("web_cp_prior") or cp.value
                n["website"] += 1
            elif r.get("web_target") == "contact_point" and apply:
                db.query(ContactPoint).filter(
                    ContactPoint.entity_id == r["entity_id"], ContactPoint.kind == "website",
        ...
        print(f"{verb}: repoint={n['repoint']} website={n['website']} deactivate={n['deactivate']}")
        if not apply:
...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(description="Final decisions cleanup (dry-run default).")
    ap.add_argument("--apply", action="store_true", help="actually write (default: dry run)")
    ap.add_argument("--undo-csv", default="undo_final_decisions_2026-07-05.csv")
    ap.add_argument("--reactivate-from", dest="undo_in", help="undo CSV to reverse a prior apply")
    ap.add_argument("--hold", default="", help="';'-separated names to hold (skip, leave as-is)")
    ap.add_argument("--overwrite-web", default="",
                    help="';'-separated names whose website should overwrite an existing value")
    args = ap.parse_args(argv)
    if args.undo_in:
        return reactivate(args.undo_in, args.apply)
    held = {_norm(s) for s in args.hold.split(";") if s.strip()}
    overwrite_web = {_norm(s) for s in args.overwrite_web.split(";") if s.strip()}
    return run(args.apply, args.undo_csv, held, overwrite_web)
