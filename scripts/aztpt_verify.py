"""Cross-reference shopping-essentials providers with AZ DOR TPT license registry.

V1.5 Trust-Signal Verifier Bundle wave 4, ticket #22 per
``outputs/v1_5_carries_inventory.md``. Mirrors the wave-3
:mod:`scripts.azcc_towing_verify` per-candidate Playwright lookup pattern.

Usage:
    python -m scripts.aztpt_verify --dry-run
    python -m scripts.aztpt_verify --limit 50
"""
...
from app.contrib.aztpt_client import fetch_aztpt_license_search  # noqa: E402
...
def _aztpt_candidate_names(entry: dict[str, Any]) -> list[str]:
    out: list[str] = []
    for key in ("business_name", "legal_name", "name"):
...
def _best_aztpt_match(
    ...
    for entry in registry:
        for cand in _aztpt_candidate_names(entry):
            score = int(
...
def _aztpt_payload(entry: dict[str, Any], score: int) -> dict[str, Any]:
    fields = (
        "tpt_license_number",
        "business_status",
        "business_name",
    ...
    *,
    ...
    with SessionLocal() as db:
        ...
        for prov in providers:
            existing = (prov.attributes or {}).get("aztpt") if prov.attributes else None
            existing_license = (
                (existing or {}).get("tpt_license_number")
            ...
            registry = fetch_aztpt_license_search(client, name=prov.provider_name)
            match = _best_aztpt_match(prov.provider_name, registry)
            if match is None:
                counts["skipped_no_match"] += 1
                logger.info(
                    "aztpt_verify no_match",
                    extra={
            ...
            entry, score = match
            license_number = str(entry.get("tpt_license_number") or "").strip()
            ...
            if dry_run:
                logger.info(
                    "aztpt_verify dry_run match",
                    extra={
                        "provider": prov.provider_name,
                        "tpt_license_number": license_number,
                        "name_score": score,
                    },
                )
                continue
            ...
            attrs = _merge_attributes(
                ...
                {"aztpt": _aztpt_payload(entry, score)},
            )
